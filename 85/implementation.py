...
BATCH_SIZE = 32
MAX_WORDS_IN_REVIEW = 150 # Maximum length of a review to consider, also called as number of steps
...
linked_verbs = set({"am", "is", "isn't", "are", "aren't", "was", "wasn't", "were", "weren't", "be", "been", "being", "act", "acts", "acted", 
  ...
  "turns", "turned", "wax", "waxed", "waxes", "went", "but", "however", "still", "yet"})
...
def preprocess(review):
    ...
    for i in range(review_length):
      if redundant_words[i] in linked_verbs:
        j = 1
        while (i + j) < review_length and j <= 3:
    ...
    for i in range(len(words)):
      if words[i] in deny_words:
        simplified_words.append("not")
        if i < 10 or i + 10 > len(words):
          keep_prob.append(1)
        else:
          keep_prob.append(random())
      if words[i] == '':
        continue
      if words[i] in linked_verbs:
        continue
      if words[i] in stop_words:
        continue
      if words[i] in abbr_words:
        continue
      else:
        simplified_words.append(words[i])
        if i < 10 or i + 10 > len(words):
          keep_prob.append(1)
        else:
          keep_prob.append(random())
